...
export const NotesPane = () => {
  ...
  return (
    <div className="h-full flex flex-col relative transition-all bg-black border border-terminal-main">
      {/* TAB SELECTOR */}
      <div className="flex border-b transition-colors border-terminal-main/30">
        {tabs.map((tab, idx) => (
          <button
            key={tab}
            ...
            className={`flex-1 py-1.5 text-[9px] font-black uppercase tracking-widest transition-all ${
              activeIndex === idx
                ? 'bg-terminal-main text-black retro-invert'
                : 'text-terminal-main/40 hover:text-terminal-main/70'
            } ${idx !== tabs.length - 1 ? 'border-r border-terminal-main/30' : ''}`}
          >
            [ {tab} ]
          </button>
        ))}
      ...
      <div className="flex flex-1 overflow-hidden min-h-[300px]">
        {/* LINE NUMBERS */}
        <div className="w-10 flex flex-col items-end pr-3 pt-4 text-[9px] font-bold select-none border-r font-mono transition-colors text-terminal-main/40 bg-black border-terminal-main/20">
        ...
        <div className="flex-1 relative group">
          <textarea 
            ...
            className="w-full h-full bg-transparent focus:outline-none p-4 pt-4 text-[11px] leading-6 resize-none custom-scrollbar selection:bg-terminal-main selection:text-black font-mono tracking-wide transition-colors text-terminal-main placeholder:text-terminal-main/20"
            placeholder={`> AWAITING_${tabs[activeIndex]}_INPUT...`}
            spellCheck={false}
          ></textarea>
          ...
          {/* CURSOR EFFECT */}
          <div className="absolute bottom-4 right-4 w-2 h-4 bg-terminal-main animate-pulse pointer-events-none shadow-[0_0_5px_rgba(255,157,0,0.5)] opacity-60"></div>
        </div>
      </div>
    </div>
  );
